...
function renderIfNotFound(){
  return templates[ !options.native() ? 'if-not-found' : 'native-if-not-found' ];
}
...
function renderModule(module){
  ...
  var sourceUrls = options.debug(),
      template   = sourceUrls ? templates['module-with-source-urls'] : templates.module;

  return format(template, {
    id       : module.id,
    content  : !sourceUrls ? module.content : JSON.stringify(module.content + '//@ sourceURL=' + module.filename),
    requires : JSON.stringify(mapOf(module))
  });
}
...
function renderNativeRequire(){
  return options.native() ? templates['native-require'] : '';
}
