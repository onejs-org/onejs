...
function contentOf(module){
  debug('Reading %s%s', module, module.component ? ' [component]' : '');

  var content = readFile(module.filename),
      uncompressed;

  if ( module.component && ( uncompressed = uncompress(content) ) ) {
...
function newModule(filename, pack, id, isEntry){
  ...
  module = {
    component : pack ? pack.component : componentAt(path.dirname(filename)),
    id        : id,
    init      : init,
